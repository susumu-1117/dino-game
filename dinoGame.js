...
function createDino() {
  game.dino = {
    x: game.image.dino.width / 2, // 恐竜の位置を示すX座標の数値で、画像の中心としてます。表示位置は左端。
    y: canvas.height - game.image.dino.height / 2, // 恐竜の位置を示すY座標の数値で、画像の中心としてます。表示位置は一番下。
    moveY: 0, // 恐竜のY軸の移動速度の数値
    width: game.image.dino.width, // 恐竜の画像の横幅の数値
    height: game.image.dino.height, // 恐竜の画像の縦幅の数値
    image: game.image.dino // 恐竜の画像のイメージオブジェクト
  }
}
...
function hitCheck() {
  for (const enemy of game.enemys) {
    if (
        Math.abs(game.dino.x - enemy.x) < game.dino.width * 0.8 / 2 + enemy.width * 0.9 / 2 && // Math.abs(数値) は数値の絶対値を計算
        ...
        ctx.fillText(`Game Over!`, 150, 200); // ctx.fillText(文章, x, y); は文章を表示させる
